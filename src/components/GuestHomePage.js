...
class BookStayButton extends React.Component {
  ...
  //Save a reversed stay to back end
    ...
    return (
      <>
        <Form onFinish={this.search} layout="inline">
          ...
          > 
            <InputNumber min={1} defaultValue={50} />
          </Form.Item>
          <Form.Item>
            <Button
              loading={this.state.loading}
...
class CancelReservationButton extends React.Component {
 ...
 //Delete a reservation from back end, if success, also remove from front end
  handleCancelReservation = async () => {
    ...
    try {
      ...
      onCancelSuccess();        //Call MyReservations's loadData to refresh reservations
    } catch (error) {
      message.error(error.message);
    } finally {
      this.setState({
...
class MyReservations extends React.Component {
  ...
  //Get reservations of this guest user from back end
  loadData = async () => {
    this.setState({
      loading: true,
    });
